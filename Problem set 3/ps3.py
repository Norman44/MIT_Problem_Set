...
def is_valid_word(word, hand, word_list):
    ...
    returns: boolean
    """
    def generate_wc_words(word):
        word_lst = []
        for v in VOWELS:
            wc_index = word.find("*")
            new_word = word[0:wc_index] + v
            if wc_index < len(word) - 1:
                new_word += word[wc_index + 1 :]
            word_lst.append(new_word.lower())
        return word_lst
    
    if '*' in word:
        words_that_replace_wildcard = generate_wc_words(word)
        not_in_list = True
...
def play_hand(hand, word_list, lap = True):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        if lap:
            letter_sub = input("Would you like to substitute a letter (yes/no)? ")
            if letter_sub.lower() == 'yes':
                lett = input("Which letter would you like to substitute? ")
                print()
...
def substitute_hand(hand, letter):
    ...
    #If letter is in the hand
    if letter in hand.keys():

        choice = True
        # choice a random letter that is not yet in the hand
        while choice:
            lett = random.choice(VOWELS + CONSONANTS)
            if lett not in hand.keys():
                hand[lett] = hand[letter]
                del hand[letter]
                choice = False
            else:
                continue
    ...
    #Else if not in the hand 
    else:
        #Keep the hand as it is
        return hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while number_of_hands > 0:
        ...
        check = 1
        while one_hand_ok:
            play_score += play_hand(hand, word_list, lap)
            if check == 1:
